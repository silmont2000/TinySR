...
from models.tinyfusion.tinysr_sd3 import MaskSD3Transformer2DModel
from utils.util import load_lora_state_dict, update_ema
...
def main(args):
    ...
    transformer = MaskSD3Transformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer", groups=groups, low_cpu_mem_usage=False)
    transformer_teacher = SD3Transformer2DModel.from_pretrained(
    ...
    transformer.gumbel_gates.requires_grad_(True)
    transformer.gumbel_gates_tea.requires_grad_(True)

    if accelerator.is_main_process:
        for name, param in transformer.named_parameters():
            if param.requires_grad:
                logger.info(f"Trainable parameter: {name} with shape {param.shape} and dtype {param.dtype}")
    vae.to(accelerator.device, dtype=weight_dtype)
    transformer.to(accelerator.device, dtype=weight_dtype)
    transformer_teacher.to(accelerator.device, dtype=weight_dtype)
    
    ema = deepcopy(transformer)  # Create an EMA of the model for use after training
        ...
        if accelerator.is_main_process:
            ...
            for index, model in enumerate(models):
                ...
                output_dir, transformer_lora_layers=transformer_lora_layers_to_save,weight_name=f"transformer.safetensors"                    
                    ...
                    torch.save(model.gumbel_gates.state_dict(), os.path.join(output_dir, f"gumbel_gates.pt"))
                    torch.save(model.gumbel_gates_tea.state_dict(), os.path.join(output_dir, f"gumbel_gates_tea.pt"))
                    torch.save(model.options, os.path.join(output_dir, f"options.pt"))
                else:
                    pass
                weights.pop()
    ...
    def load_model_hook(models, input_dir):
        ...
        transformer.gumbel_gates.load_state_dict(torch.load(os.path.join(input_dir, "gumbel_gates.pt")))
        transformer.options = torch.load(os.path.join(input_dir, "options.pt"))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                with autocast_ctx:
                    tau_range = [4,0.1]
                    scaling_range = [1e2, 1e2]
                    tau = tau_range[0] + (tau_range[1] - tau_range[0]) * step / max(args.max_train_steps - 1, 1)
                    scaling = scaling_range[0] + (scaling_range[1] - scaling_range[0]) * step / max(args.max_train_steps - 1, 1)
                    transformer.module.scaling = scaling
                    transformer.module.tau = tau
                    with torch.no_grad():
                        ...
                        model_pred_tea = transformer_teacher(
                        ...
                        latent_tea =  model_input - model_pred_tea 
                    
                    timesteps = torch.tensor([1000.], device=model_input.device, dtype=weight_dtype)
                    ...
                    image_stu = vae.decode(latent_stu / vae.config.scaling_factor, return_dict=False)[0]
                    
                    image_stu = (image_stu * 0.5 + 0.5).clamp(0, 1)
                    hr_values = (hr_values * 0.5 + 0.5).clamp(0, 1)
                    
                    lpips_loss = lpips(image_stu, hr_values) 
                    loss_g = lpips_loss + l1_loss
                ...
                update_ema(ema, transformer.module, decay=args.ema_decay)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 500:
                        ...
                        torch.save(ema.gumbel_gates.state_dict(), os.path.join(save_path, f"ema_gumbel_gates.pt"))
                        torch.save(ema.gumbel_gates_tea.state_dict(), os.path.join(save_path, f"ema_gumbel_gates_tea.pt"))
                        torch.save(ema.options, os.path.join(save_path, f"ema_options.pt"))
            ...
            if accelerator.is_main_process and step % 2000 == 49:
                with autocast_ctx:
                    log_dict["image_stu"] = (image_stu * 2 - 1).clamp(-1,1).cpu()
