...
def _gaussian_weights(tile_width, tile_height, nbatches, device, in_channels, dtype):
    # Smooth blend: matches models/quant/tiler.py (var=0.2) to avoid circular
    # artifacts from a too-peaked window when tiling large latents.
    var = 0.2
    ...
    # Keep float32: fp16 underflows at tile edges and makes contributors zero -> 0/0 NaN.
    return torch.tile(torch.tensor(weights, device=device, dtype=torch.float32), (nbatches, in_channels, 1, 1))
...
def tile_sample(lq_latent, transformer, timesteps, pooled_prompt_embeds, weight_dtype,
    ...
    noise_pred = noise_pred / contributors.clamp_min(1e-6)
    return noise_pred.to(lq_latent.device, dtype=weight_dtype)
...
def run_nunchaku_benchmark(args, transformer, vae, timesteps, pooled_prompt_embeds, weight_dtype, device):
    ...
    def main(pixel_values):
        with torch.no_grad():
            ...
            image = vae.decode(
                (latent_stu / vae.config.scaling_factor).to(dtype=weight_dtype),
                return_dict=False,
...
if __name__ == "__main__":
    ...
    while img_idx < datalen:
        ...
        with torch.no_grad():
            ...
            ls = mi - mp
            images = vae.decode(
                (ls / vae.config.scaling_factor).to(dtype=weight_dtype),
                return_dict=False,
            )[0].clamp(-1, 1)
        if device.type == "cuda":
            torch.cuda.synchronize()
